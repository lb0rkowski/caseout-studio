...
// ─── EXTRACTED COMPONENTS (outside main fn to avoid re-creation) ───

function SlotPicker({date,hours,onToggle,canToggle,accentColor,bookedSet}:{date:string;hours:number[];onToggle:(h:number)=>void;canToggle:(h:number)=>boolean;accentColor:string;bookedSet:Set<number>}){
  ...
  return<div className="space-y-1">{Array.from({length:sh[1]-sh[0]},(_,i)=>{
    const h=sh[0]+i;const isBk=bookedSet.has(h);const isSel=hours.includes(h);const ok=canToggle(h);
...
export default function RezerwacjePage(){
  ...
  const[formType,setFormType]=useState("recording");
  ...
  useEffect(()=>{
    if(typeof window!=="undefined"){
      ...
      if(pkgId){const found=PACKAGES.find(x=>x.id===pkgId);if(found){setMode("package");setPkg(found);}}
      if(p.get("payment")==="success")setConfirm({orderNumber:"OPLACONE",sessions:[]});
    }
  },[]);
  ...
  const getBooked=(date:string):Set<number>=>{
  ...
  const onPhoneChange=(v:string)=>{
    const f=formatPhone(v);setFormPhone(f);
    const d=f.replace(/\D/g,"");
    if(d.length>0&&d.length<9)setFieldErr(p=>({...p,phone:"9 cyfr"}));
    else setFieldErr(p=>{const n={...p};delete n.phone;return n;});
  };
  const onEmailChange=(v:string)=>{
    setFormEmail(v);
    if(v.length>0&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v))setFieldErr(p=>({...p,email:"Nieprawidlowy email"}));
    else setFieldErr(p=>{const n={...p};delete n.email;return n;});
  };
  const onNameChange=(v:string)=>{
    setFormName(v);
    if(v.length>0&&v.trim().length<3)setFieldErr(p=>({...p,name:"Min. 3 znaki"}));
    else if(v.length>0&&!v.trim().includes(" "))setFieldErr(p=>({...p,name:"Podaj imie i nazwisko"}));
    else setFieldErr(p=>{const n={...p};delete n.name;return n;});
  };
  const validate=()=>{
    const fe:Record<string,string>={};
    if(!formName||formName.trim().length<3)fe.name="Podaj imie i nazwisko";
    else if(!formName.trim().includes(" "))fe.name="Podaj imie i nazwisko";
    if(!formEmail||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formEmail))fe.email="Podaj prawidlowy email";
    if(!formPhone||formPhone.replace(/\D/g,"").length!==9)fe.phone="9 cyfr";
  ...
  // ─── SINGLE: slot toggling ───
  const toggleHour=(h:number)=>{
    if(!selDate)return;if(getBooked(selDate).has(h))return;
  ...
  const canSel=(h:number):boolean=>{
    if(!selDate||getBooked(selDate).has(h))return false;if(selHours.length===0)return true;if(selHours.includes(h))return true;
    const mn=Math.min(...selHours),mx=Math.max(...selHours);return h===mx+1||h===mn-1;
  };

  // ─── PACKAGE: multi-day toggling ───
  ...
  const pkgToggle=(date:string,h:number)=>{
    if(getBooked(date).has(h))return;
    ...
    if(current.includes(h)){
      ...
      else if(h===current[0]&&current.length>1)setPkgSlots({...pkgSlots,[date]:current.slice(1)});
      return;
    }
    ...
    else setPkgSlots({...pkgSlots,[date]:[h]});
  };
  const pkgCanSel=(date:string,h:number):boolean=>{
    if(getBooked(date).has(h))return false;
    const current=pkgSlots[date]||[];
  ...
  const pkgSessionList=Object.entries(pkgSlots).filter(([,hrs])=>hrs.length>0).sort(([a],[b])=>a.localeCompare(b)).map(([date,hrs])=>({date,hour:Math.min(...hrs),duration:hrs.length}));
  ...
  const sendEmail=async(orderNumber:string,sessions:{date:string;hour:number;duration:number}[],totalPrice:number,payMethod:string)=>{
    try{await fetch("/api/email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:formEmail,orderNumber,name:formName,sessions,totalPrice,packageName:pkg?.name||null,payMethod})});}catch(e){console.error("Email:",e);}
  };
  ...
  const bookSingle=async(payMethod:"payu"|"onsite")=>{
    ...
    const we=isWeekendDate(selDate);const price=calcPrice(dur,we);
    ...
    setBusy(true);setErr("");
    const notesStr=formNotes+(formNotes?" | ":"")+"Cena: "+price+" zl | na miejscu";
    const result=await addBooking({date:selDate,hour:startH,duration:dur,type:formType,name:formName,email:formEmail,phone:formPhone,notes:notesStr,status:"confirmed"});
    if(!result.booking){setBusy(false);setErr(result.error||"Blad");return;}
    const orderNum=(result.booking as any).order_number||"CS-"+Date.now();
    await sendEmail(orderNum,[{date:selDate,hour:startH,duration:dur}],price,"onsite");
    setConfirm({orderNumber:orderNum,sessions:[{date:selDate,hour:startH,duration:dur}]});
    setBusy(false);setStep(0);setSelDate(null);setSelHours([]);resetForm();
  };
  ...
  const bookPackage=async(payMethod:"payu"|"onsite")=>{
    ...
    setBusy(true);setErr("");
    const notesStr=formNotes+(formNotes?" | ":"")+"Pakiet: "+pkg.name+" | "+pkg.price+" zl | na miejscu";
    try{
      const res=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sessions:pkgSessionList,type:formType,name:formName,email:formEmail,phone:formPhone,notes:notesStr,package_id:pkg.id})});
      const data=await res.json();
      if(!res.ok){setBusy(false);setErr(data.error||"Blad");return;}
      const orderNum=data.order_number||"CS-"+Date.now();
      await sendEmail(orderNum,pkgSessionList,pkg.price,"onsite");
      setConfirm({orderNumber:orderNum,sessions:pkgSessionList,pkg});
      setBusy(false);setPkgSlots({});setStep(0);resetForm();
    }catch(e:any){setBusy(false);setErr(e.message);}
  };

  const resetForm=()=>{setFormName("");setFormEmail("");setFormPhone("");setFormType("recording");setFormNotes("");setFieldErr({});};
  ...
  // ─── INLINE FORM JSX (not a component!) ───
  const formFieldsJsx=<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
    <div><label className="font-mono text-[11px] text-cs-dim tracking-[0.15em] mb-2 block">TYP SESJI</label><select className={inp+" "+okB+" cursor-pointer"} value={formType} onChange={e=>setFormType(e.target.value)}>{SESSION_TYPES.map(t=><option key={t.id} value={t.id}>{t.icon} {t.name}</option>)}</select></div>
    <div><label className="font-mono text-[11px] text-cs-dim tracking-[0.15em] mb-2 block">IMIE I NAZWISKO *</label><input className={inp+" "+(fieldErr.name?errB:okB)} value={formName} onChange={e=>onNameChange(e.target.value)} placeholder="Jan Kowalski"/>{fieldErr.name&&<div className="font-mono text-[11px] text-cs-red mt-1.5">{fieldErr.name}</div>}</div>
    <div><label className="font-mono text-[11px] text-cs-dim tracking-[0.15em] mb-2 block">EMAIL *</label><input className={inp+" "+(fieldErr.email?errB:okB)} type="email" value={formEmail} onChange={e=>onEmailChange(e.target.value)} placeholder="email@domena.pl"/>{fieldErr.email&&<div className="font-mono text-[11px] text-cs-red mt-1.5">{fieldErr.email}</div>}</div>
    <div><label className="font-mono text-[11px] text-cs-dim tracking-[0.15em] mb-2 block">TELEFON *</label><input className={inp+" "+(fieldErr.phone?errB:okB)} value={formPhone} onChange={e=>onPhoneChange(e.target.value)} placeholder="XXX XXX XXX" maxLength={11} inputMode="numeric"/>{fieldErr.phone?<div className="font-mono text-[11px] text-cs-red mt-1.5">{fieldErr.phone}</div>:<div className="font-mono text-[10px] text-cs-dim mt-1">{formPhone.replace(/\D/g,"").length}/9</div>}</div>
    <div className="md:col-span-2"><label className="font-mono text-[11px] text-cs-dim tracking-[0.15em] mb-2 block">UWAGI</label><input className={inp+" "+okB} value={formNotes} onChange={e=>setFormNotes(e.target.value)} placeholder="opcjonalnie"/></div>
  </div>;
  ...
  return(
    <div className="pt-20 md:pt-28"><Sect>
      ...
      {/* MODE SWITCH */}
      {!loading&&!confirm&&step===0&&!pkgEditDate&&<div className="flex gap-2 mb-6 flex-wrap">
      ...
      {!loading&&!confirm&&<>
        ...
        <div className="flex justify-between items-center mb-5 flex-wrap gap-4">
          ...
          {(selDate||pkgEditDate)&&step!==0&&<button onClick={()=>{setSelDate(null);setSelHours([]);setPkgEditDate(null);setStep(0);setErr("");}} className="font-mono text-[11px] text-cs-gold-dim hover:text-cs-gold cursor-pointer bg-transparent border border-cs-line px-4 py-2 rounded-sm transition-colors">&larr; Wroc do kalendarza</button>}
        </div>
        ...
        {mode==="single"&&<>
          ...
          {/* Calendar */}
          {step===0&&<div className="bg-cs-card border border-cs-line rounded-sm overflow-hidden"><div className="grid grid-cols-7">
            ...
            {Array.from({length:dim},(_,i)=>{
              ...
              const isT=today===ds,past=new Date(ds)<new Date(today),isSat=di===5;
              const rate=isSat?HOURLY_RATE+WEEKEND_SURCHARGE:HOURLY_RATE;
              return<div key={day} onClick={()=>!isSun&&!past&&(()=>{setSelDate(ds);setSelHours([]);setStep(1);setErr("");})()}className="min-h-[70px] md:min-h-[100px] p-1.5 md:p-2.5 border-b border-cs-line border-r border-r-[rgba(26,31,43,0.05)] transition-colors hover:bg-[rgba(196,151,103,0.015)]" style={{opacity:past?0.3:isSun?0.4:1,cursor:isSun||past?"default":"pointer"}}>
                <div className="font-body text-sm md:text-base font-semibold w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center mb-1" style={{color:isT?"#C49767":isSun?"#403830":"#D8D0C6",background:isT?"rgba(196,151,103,0.1)":"transparent"}}>{day}</div>
          ...
          {step===1&&selDate&&<RevealDiv><div className="bg-cs-card border border-cs-line rounded-sm p-5 md:p-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-3">
              <div><div className="font-mono text-[11px] text-cs-gold-dim tracking-[0.2em] mb-1">WYBIERZ GODZINY</div><h3 className="font-display text-xl md:text-2xl text-cs-white uppercase">{selDate}</h3></div>
              {selHours.length>0&&<div className="text-right"><div className="font-display text-2xl text-cs-gold">{Math.min(...selHours)}:00 - {Math.max(...selHours)+1}:00</div><div className="font-mono text-xs text-cs-dim">{selHours.length}h = <span className="text-cs-gold font-bold">{singleTotal} zl</span></div></div>}
            </div>
            <SlotPicker date={selDate} hours={selHours} onToggle={toggleHour} canToggle={canSel} accentColor="#C49767" bookedSet={getBooked(selDate)}/>
          ...
          {step===2&&selDate&&<RevealDiv><div className="bg-cs-card border border-[rgba(144,113,79,0.15)] rounded-sm p-6 md:p-10 relative overflow-hidden">
            ...
            <div className="font-mono text-[11px] text-cs-gold-dim tracking-[0.2em] mb-4">// DANE</div>
            {formFieldsJsx}
          ...
          {step===3&&selDate&&<RevealDiv><div className="bg-cs-card border border-[rgba(144,113,79,0.2)] rounded-sm p-6 md:p-10 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="p-4 rounded-sm" style={{background:"rgba(5,8,16,0.5)",border:"1px solid #1A1F2B"}}>
                ...
                <div className="font-mono text-[10px] text-cs-dim mt-3">KLIENT</div>
                <div className="font-body text-base text-cs-text">{formName}</div>
                <div className="font-mono text-[11px] text-cs-dim">{formEmail} | {formPhone}</div>
              </div>
              <div className="p-4 rounded-sm" style={{background:"rgba(5,8,16,0.5)",border:"1px solid #1A1F2B"}}>
        ...
        {mode==="package"&&pkg&&<>
          <div className="mb-5 p-4 rounded-sm" style={{background:"rgba(100,180,255,0.04)",border:"1px solid rgba(100,180,255,0.15)"}}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div><div className="font-mono text-[11px] tracking-[0.2em] mb-1" style={{color:"#64B4FF"}}>PAKIET {pkg.name.toUpperCase()}</div><div className="font-body text-base text-cs-muted">{pkg.desc} | {pkg.discount} taniej</div></div>
              <div className="text-right"><div className="font-display text-2xl font-bold" style={{color:"#64B4FF"}}>{pkgUsed} / {pkgBudget}h</div><div className="font-mono text-xs text-cs-dim">{pkgRemaining>0?pkgRemaining+"h pozostalo":"Gotowe!"}</div></div>
            </div>
            <div className="mt-3 h-2 rounded-full overflow-hidden" style={{background:"rgba(100,180,255,0.1)"}}><div className="h-full rounded-full transition-all duration-500" style={{width:(pkgUsed/pkgBudget*100)+"%",background:"linear-gradient(90deg, #64B4FF, #4090E0)"}}/></div>
          </div>

          {/* Package calendar */}
          {step===0&&!pkgEditDate&&<>
            <div className="bg-cs-card border border-cs-line rounded-sm overflow-hidden"><div className="grid grid-cols-7">
              {DN.map(d=><div key={d} className="py-3 px-1 text-center font-mono text-[11px] md:text-xs text-cs-dim border-b border-cs-line font-bold">{d}</div>)}
              {Array.from({length:fd}).map((_,i)=><div key={"e"+i} className="min-h-[70px] md:min-h-[100px] border-b border-cs-line border-r border-r-[rgba(26,31,43,0.05)]"/>)}
              {Array.from({length:dim},(_,i)=>{
                const day=i+1,ds=yr+"-"+String(mo+1).padStart(2,"0")+"-"+String(day).padStart(2,"0");
                ...
                const pkgH=pkgSlots[ds]||[];const hasPkg=pkgH.length>0;
                return<div key={day} onClick={()=>{if(!isSun&&!past){setPkgEditDate(ds);}}} className="min-h-[70px] md:min-h-[100px] p-1.5 md:p-2.5 border-b border-cs-line border-r border-r-[rgba(26,31,43,0.05)] transition-colors hover:bg-[rgba(196,151,103,0.015)]" style={{opacity:past?0.3:isSun?0.4:1,cursor:isSun||past?"default":"pointer",background:hasPkg?"rgba(100,180,255,0.04)":"transparent"}}>
          ...
          {pkgEditDate&&step===0&&<RevealDiv><div className="bg-cs-card border border-cs-line rounded-sm p-5 md:p-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-3">
              <div><div className="font-mono text-[11px] tracking-[0.2em] mb-1" style={{color:"#64B4FF"}}>WYBIERZ GODZINY — {pkgEditDate}</div><div className="font-mono text-xs text-cs-dim">Pozostalo: {pkgRemaining}h z {pkgBudget}h</div></div>
              {(pkgSlots[pkgEditDate]||[]).length>0&&<div className="text-right"><div className="font-display text-xl" style={{color:"#64B4FF"}}>{Math.min(...pkgSlots[pkgEditDate])}:00 - {Math.max(...pkgSlots[pkgEditDate])+1}:00</div><div className="font-mono text-xs text-cs-dim">{pkgSlots[pkgEditDate].length}h</div></div>}
            </div>
            <SlotPicker date={pkgEditDate} hours={pkgSlots[pkgEditDate]||[]} onToggle={(h)=>pkgToggle(pkgEditDate,h)} canToggle={(h)=>pkgCanSel(pkgEditDate,h)} accentColor="#64B4FF" bookedSet={getBooked(pkgEditDate)}/>
          ...
          {step===2&&<RevealDiv><div className="bg-cs-card border border-[rgba(100,180,255,0.15)] rounded-sm p-6 md:p-10 relative overflow-hidden">
            ...
            <div className="font-mono text-[11px] tracking-[0.2em] mb-4" style={{color:"#64B4FF"}}>// DANE — PAKIET {pkg.name.toUpperCase()}</div>
            {formFieldsJsx}
            {err&&<div className="font-mono text-sm text-cs-red mt-5 p-3 rounded-sm" style={{background:"rgba(139,48,48,0.06)",border:"1px solid rgba(139,48,48,0.15)"}}>{err}</div>}
          ...
          {step===3&&<RevealDiv><div className="bg-cs-card border border-[rgba(100,180,255,0.2)] rounded-sm p-6 md:p-10 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="p-3 rounded-sm" style={{background:"rgba(5,8,16,0.5)",border:"1px solid #1A1F2B"}}><div className="font-body text-base text-cs-text">{formName}</div><div className="font-mono text-[11px] text-cs-dim">{formEmail} | {formPhone}</div></div>
              </div>
              <div>
                <div className="font-mono text-[10px] text-cs-dim mb-2">CENA</div>
