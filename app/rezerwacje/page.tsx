...
export default function RezerwacjePage(){
  ...
  const clickSlot=(day:number,hour:number)=>{
    ...
    if(isBooked(ds,hour))return;setSelDate(ds);setSelSlot(hour);setStep(1);setErr("");setFieldErr({});
  };
  ...
  const book=async()=>{
    if(!validate()) return;
    if(!selDate||selSlot===null){setErr("Wybierz termin");return;}

    setBusy(true);setErr("");
    const result=await addBooking({date:selDate!,hour:selSlot!,duration:form.duration,type:form.type,name:form.name,email:form.email,phone:form.phone,notes:form.notes,status:"confirmed"});
    setBusy(false);
    if(result.booking){
      setConfirm(result.booking);setStep(0);setSelDate(null);setSelSlot(null);
      setForm({name:"",email:"",phone:"",type:"recording",duration:2,notes:""});setFieldErr({});
    } else {
    ...
    <div className="pt-20 md:pt-28"><Sect>
      ...
      {confirm&&<RevealDiv><div className="bg-[rgba(59,107,59,0.06)] border border-[rgba(59,107,59,0.2)] rounded-sm p-8 md:p-10 mb-8 text-center">
        ...
        <div className="flex gap-3 justify-center mt-6">
          ...
          <a href={makeGCalUrl(confirm)} target="_blank" rel="noopener noreferrer">
            <GlowBtn>Dodaj do Google Calendar</GlowBtn>
          </a>
        </div>
      </div></RevealDiv>}
      ...
      {!loading&&<>
        ...
        {selDate&&step===0&&<RevealDiv className="mt-6"><div className="bg-cs-card border border-cs-line rounded-sm p-6 md:p-8">
          <h3 className="font-display text-lg md:text-xl text-cs-white uppercase mb-4"><span className="font-mono text-[11px] text-cs-gold-dim mr-3">DOSTEPNE:</span>{selDate}</h3>
        ...
        {step===1&&<RevealDiv className="mt-6"><div className="bg-cs-card border border-[rgba(144,113,79,0.15)] rounded-sm p-6 md:p-10 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div><label className="font-mono text-[11px] text-cs-dim tracking-[0.15em] mb-2 block">TYP SESJI</label><select className={inp+" "+okBorder+" cursor-pointer"} value={form.type} onChange={e=>setForm({...form,type:e.target.value})}>{SESSION_TYPES.map(t=><option key={t.id} value={t.id}>{t.icon} {t.name}</option>)}</select></div>
            <div><label className="font-mono text-[11px] text-cs-dim tracking-[0.15em] mb-2 block">CZAS TRWANIA</label><select className={inp+" "+okBorder+" cursor-pointer"} value={form.duration} onChange={e=>setForm({...form,duration:parseInt(e.target.value)})}>{DURATIONS.map(d=><option key={d.hours} value={d.hours}>{d.label}</option>)}</select></div>
            <div>
          ...
          <div className="flex gap-4 mt-8"><GlowBtn onClick={book} disabled={busy||Object.keys(fieldErr).length>0}>{busy?"Wysylanie...":"Potwierdz Rezerwacje"}</GlowBtn><GlowBtn ghost onClick={()=>{setStep(0);setSelSlot(null);setErr("");setFieldErr({});}}>Anuluj</GlowBtn></div>
        </div></RevealDiv>}
      </>}
    </Sect></div>
  );
